...
public class DateUtils {
    ...
    public static String formatDate(Date date, Object... pattern) {
        if (pattern != null && pattern.length > 0) {
            ...
            return DateFormatUtils.format(date, DateUtils.DATE_FORMAT);
        }
    }
    ...
    public static String getUTCTimeStr() {
        StringBuilder uctTimeBuilder = new StringBuilder();
        // 1、取得本地时间：
        ...
        uctTimeBuilder.append(year).append("-").append(month).append("-").append(day) ;
        uctTimeBuilder.append(" ").append(hour).append(":").append(minute) ;
        try{
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");
            sdf.parse(uctTimeBuilder.toString()) ;
            return uctTimeBuilder.toString() ;
        }catch(ParseException e)
        {
            e.printStackTrace() ;
        }
